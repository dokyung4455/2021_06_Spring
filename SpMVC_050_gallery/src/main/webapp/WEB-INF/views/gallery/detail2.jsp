...
<style>
	...
	div#galler_box div:last-of-type{
		flex:3;
		height:200px;
	}
	div#gallery_files {
	...
	div.gallery_file:after {
		...
		transition:1s;
		/*
		box 내의 text의 그려지는 높이를 box의 높이와 같게 만들면 text가 box의 세로방향 가운데 정렬이 된다.
		*/
		line-height: 200px;
		text-align:center; 
...
update_button.addEventListener("click",()=>{
	...
	만약 GALLER.g_seq 값이 S0001 등과 같이 문자열 형 이라면 이 코드는 JS 문법오류가 발생할 것이다.
	
	현재 작성한 코드는 JSP 코드이다
	이 코드는 Response가 될때 HTML코드로 변환이 되고 Script부분도 변환이 되는데
	
	변환되는 과정에서 ${GALLERY.g_seq} 는 담겨있는 문자열인 S0001만 단독으로 나타난다.
	
	아래코드는 "/root-context/gallery/update?g_seq=" + S0001 처럼 변환이 된다.
			
	결국 JS 코드가 실행될때 + S0001 처럼 변환되어 변수를 찾게 된다.
	그리고 S0001 이라는 변수가 정의되지 않아 문법 오류가 발생한다.
	
	EL tag를 사용하여 스크립트 부분에서 직접 값을 부착할때는 DQ("") 부착하여 문법 오류를 방지하자
...
/*
 ...
 논리적인 오류를 일으킬수있다. 그러한 문제를 방지하기 위하여 const 키워드를 상당히 권장한다.
...
if(gallery_files) {
	gallery_files.addEventListener("click", (e)=> {
		...
		// 조건이 false가 될 수있다.
		// className.includes() 함수를 사용하여 조건 검사를 하는것이 좋다.
		alert(tag.tagName)
