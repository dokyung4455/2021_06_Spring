...
public class MemberController {
	...
	public String login(MemberVO memberVO, Model model, HttpSession session) {
		...
		if(memberVO == null) {
			...
			// 사용자 ID, PW 가 정상이면 HttpSession에 사용자 정보가 담긴 memberVO를 속성으로 세팅한다.
			session.setAttribute("MEMBER", memberVO);
			
			/*
			 * HttpSession에 속성으로 setting된 값은 어떠한 type이라도 상관없다
			 * 하지만 HttpSession에 담긴 속성은 임의로 삭제하거나, 초기화하거나 서버가 멈추거나, 일정 조건이
			 * 성립되지 않으면 서버 메모리에 영구히 남아있다.
			 * 
			 * 1. 가급정 작은 크기에 데이터만 담아라
			 * 2. 필요없으면 반드시 소멸시켜라
			 * 3. 자동 소멸되는 조건을 잘 지켜라
			 */
			 return "redirect:/";
