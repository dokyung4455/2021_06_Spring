...
public class FileServiceImplV2 extends FileServiceImplV1 {
	...
	public int delete(String imgFileName) {
		...
		/*
		 * fileService.delete(파일명)을 호출할때 파일명이 null이거나 없으면 진행을 멈추는 코드
		 * 이러한 코드는 호출하는 곳에서
		 * 
		 ...
		 * }
		 * 처럼 호출 할 수 있지만,
		 * delete() method 사용하는 곳이 여러곳 이라면 그때마다 검사하는 코드를 부착해야 한다.
		 * 
		 * 그것보다는 delete() method가 시작될때 파일의 null 값 등을 검사하여 실행을 취소하는 방법이 더 편리한 코드가 될것이다.
		 * 
		 */
		...
		if(delFile.exists()) {
			...
			if(ok) {
				...
				log.debug("파일 삭제 실패");
				/*
				 *  method return type을 wrapper class(Integer, Long)등으로 설정하면 실패하는 메시지를 return할때 null을 return 하면된다.
				 *  
				 *  primitive 숫자형으로 return type을 설정한 경우
				 *  null 값을 return 할 수 없다.
				 *  
				 *  이럴때 성공한 메시지는 양의 정수값을 return 하고 실패한 메시지는 음의 정수값을 return한다
				 *  
				 *  호출한 곳에서 성공, 실패를 검사할때
				 *  if(ref > 0) 성공
				 *  if(ref < 0) 실패
