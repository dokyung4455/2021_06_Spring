...
public class GalleryServiceImplV1 implements GalleryService{
	...
	/*
	 * @Autowired가 설정된 변수, method, 객체 등을 만나면 Spring framework는 변수를 초기화,
	 * method를 실행하여 또 변수 초기화 이미 생성되어 준비된 객체에 주입 등을 수행한다.
	 * 
	 */
	
	@Autowired
	public void create_table(GalleryDao gaDao, FileDao fiDao) {
		Map<String, String> maps = new HashMap<String,String>();
		gaDao.create_table(maps);
		fiDao.create_table(maps);
	}
	...
	public void input(GalleryDTO gaDTO, MultipartFile one_file, MultipartHttpServletRequest m_file) throws Exception {
		...
		// 대표이미지 업로드 되면 이미지를 서버에 저장하고 저장된 파일의 이름을 리턴받기
		String strUUID = fService.fileUp(one_file);
		
		// DTO에 이미지 이름 저장하기
		gaDTO.setG_image(strUUID);
		// GalleryDTO에 담긴 데이터를 tbl_gallery table에 insert 하기
		// mapper에서 insert를 수행한 후 새로 생성된 g_seq값을 selectKey 하여 gaDTO의 g_seq 변수에 담아놓은 상태이다.
		gaDao.insert(gaDTO);
		...
		// 업로드된 멀티파일을 서버에 업로드 하고 원래 파일이름과 UUID가 첨가된 파일이름을 추출하여 FileDTO에 담고
		// 다시 List에 담아 놓는다.
