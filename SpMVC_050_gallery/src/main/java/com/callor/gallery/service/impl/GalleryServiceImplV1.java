...
/*
 *  final로 선언된 Inject 변수의 초기화를 시키는데 필요한 생성자를 자동으로 만들어 주는 lombok의 기능이다.
 *  
 *  클래스를 상속하면 @Require...Con 는 상속받은 클래스에서 사용 불가
 */
...
public class GalleryServiceImplV1 implements GalleryService {
	...
	public int file_delete(Long g_seq) {
		...
		// file을 삭제하기 위해 저장된 파일 정보를 SELECT 하기
	...
	//	pagNum를 매개변수로 받아서 selectAll 한 데이터를 잘라내고 pageNum에 해당하는 List 부분만 return 하기
	//	한 페이지에 보여줄 list = 10개
	@Override
...
//		model.addAttribute("PAGE_INFO",pageDTO);
	...
	@Override
	public List<GalleryDTO> findBySearchPage(int pageNum, String search_text, String search_column, Model model) {
		// TODO Auto-generated method stub
