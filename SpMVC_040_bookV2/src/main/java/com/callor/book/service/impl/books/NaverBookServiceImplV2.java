 ...
 *  
 *  따라서 NaverServiceImplV2 클래스에는 queryURL(), jsonString(), getNaverList() method가 모두 있는 것과 같다.
 *
 *
 */
 ...
 * getNaverList() method를 재 정의 하려고 한다.
 * 
 * queryURL(), getJsonString() method는 그대로 상속받아 사용하고 getNaverList() 만 다시 작성하겠다.
 * 
 * ServiceV1의 getNaverList()는 Json-simple을 사용하여 JSON parsing을 수행한 후 lIst을 return 하는 코드
 * 
 * Service2에서는 getNaverList() method를 다시 작성하여 gson을 사용해 JSON parsing을 수행하는 코드로 작성하기
 */
@Slf4j
...
public class NaverBookServiceImplV2 extends NaverBookServiceImplV1 {
	...
	public List<BookDTO> getNaverList(String jsonString) throws ParseException {
		...
		/* List는 interface인데 interface는 자신의 type을 가지고 있지 않은 객체인데
		 * Gson을 이용하여 JSON parsing을 할때 List<DTO> 구조를 알 수 있는 방법이 없어서
		 * Gson이 특별한 객체 생성자를 하나 제공해 주고 이 객체를 통해
		 * List<DTO>의 구조를 알 수 있도록 만들어준다.
		 */
