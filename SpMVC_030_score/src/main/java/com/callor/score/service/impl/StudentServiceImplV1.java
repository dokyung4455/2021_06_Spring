...
public class StudentServiceImplV1 implements StudentService{
	...
	public String makeStNum(String prefix) {
		...
		/*
		 * prefix 만큼의 문자열을 건너뒤고 나머지 부분을 추출하기
		 * stNum = "20210010" 이고
		 * prefix = "2021"이라면
		 * stSeq = stNum.substring(4) 이런 형식의 코드가 생성되어
		 * stSeq에는 0010 문자열만 남는다.
		 */
...
//		stVO.setSt_num(newStNum);
	...
	public String scoreInput(ScoreInputVO scInputVO) {
		 ...
		 *  
		 *  그랬더니 transactionManager에 의해서 모든 Insert Or Updatr가 Rollback 되어 버렸다.
		 */
		
		// 이유를 불문하고 Exception을 발생시켜라
