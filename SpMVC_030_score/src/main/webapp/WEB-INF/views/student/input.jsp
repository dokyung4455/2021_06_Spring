...
<body>
	...
	doc.querySelector("form#student_input").addEventListener("click", (e)=> {
		...
		if(target.tagName === "BUTTON") {
			/*
			tag에 class를 지정할때 1개만 지정 할 수 있지만 다수의 class를 지정하는 경우도 있다.
			이때는 if(target.className === "className") 형식으로 class를 검사 할 수 없다.
			이럴때는 includes를 사용하여 포함된 클래스를 검사해야한다.
			*/
			if(target.className.includes("save")) {
				// 여기에 event 처리 코드를 작성 할 수 있지만 if문 내에 코드가 다소 복잡해 지므로 함수를 먼저 등록하고 함수를 호출하는 방식으로 작성
				student_submit();
